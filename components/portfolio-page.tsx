    ...
    <div className="relative min-h-screen overflow-hidden bg-[radial-gradient(circle_at_top_left,_rgba(99,102,241,0.18),_transparent_22%),radial-gradient(circle_at_top_right,_rgba(34,211,238,0.12),_transparent_20%),linear-gradient(180deg,#060816_0%,#050814_35%,#02040b_100%)] text-white">
      ...
      <main>
        ...
        <SectionBlock id="projects">
          ...
          <div className="mt-10 grid gap-6 xl:grid-cols-2">
            {projects.map((project, index) => (
              <motion.div key={project.name} initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.2 }} transition={{ duration: 0.55, delay: index * 0.08 }} className="group relative">
        ...
        <SectionBlock id="contact">
          ...
          <div className="mt-6 grid gap-6 grid-cols-1 md:grid-cols-2 items-start md:items-center">
            {/* LEFT: descriptive / consultation content (keeps grouped and compact) */}
            <div className="space-y-4">
              <div className="rounded-3xl border border-white/8 bg-white/5 p-4">
                <p className="font-medium text-white leading-6">Whether it's discussing DSA problems, mathematical concepts, or building powerful web applications — I'm all ears. Contact me for a free consultation.</p>
              </div>

              <Card className="border-white/10 bg-white/5">
                <CardContent className="p-6">
                  <p className="text-sm uppercase tracking-[0.28em] text-white/35">Social</p>
                  <div className="mt-4 space-y-3">
                    {socialLinks.map((link) => {
                      ...
                      return (
                        <a key={link.label} href={link.href} target={link.label === 'Email' ? undefined : '_blank'} rel={link.label === 'Email' ? undefined : 'noreferrer'} className="group flex items-center gap-4 rounded-2xl border border-white/10 bg-white/5 p-3 transition hover:-translate-y-0.5 hover:border-cyan-300/20 hover:bg-white/10">
                          <div className="grid h-10 w-10 place-items-center rounded-lg bg-white/5 text-cyan-200">{icon}</div>
                          <div>
                            <p className="font-medium text-white">{link.label}</p>
                            <p className="text-sm text-white/45">{link.handle}</p>
                          </div>
              ...
              <Card className="border-white/10 bg-white/5">
                <CardContent className="p-6">
                  <p className="text-sm uppercase tracking-[0.28em] text-white/35">Work style</p>
                  <div className="mt-4 flex flex-wrap gap-3">
                    {['Backend-first', 'Scalable APIs', 'Clean architecture', 'System design', 'Performance'].map((item) => (
                      <Badge key={item} className="border-white/10 bg-white/5 px-3 py-2 text-white/75">{item}</Badge>
                    ))}
                  </div>
                </CardContent>
              </Card>
            ...
            {/* RIGHT: warning + contact form (keeps compact and vertically centered) */}
            <div className="flex w-full items-center justify-center">
              <div className="w-full max-w-lg">
                <div className="mb-4 rounded-xl border border-amber-400/20 bg-amber-400/6 px-3 py-2 text-sm text-amber-200">⚠️ Note: This form may not be functional due to backend configuration. Feel free to reach out directly via email or contact number.</div>
